...
import { Search, X, SlidersHorizontal, Bell, BookmarkPlus, Shuffle, TrendingUp, Clock, ArrowDownWideNarrow } from 'lucide-react';
...
export const FilterBar: React.FC<FilterBarProps> = ({ filters, setFilters, onMenuClick, onRefresh, onSaveBoard }) => {
  ...
  const handleSortChange = (sort: SortOption) => {
      ...
      // We trigger refresh immediately via effect in App, or we could manually call onRefresh here if needed.
      // Assuming App's useEffect handles the refresh when filters change (except for search text which usually waits for enter).
      // However, App.tsx only watches [filters.persons, filters.sources, filters.tags]. 
      // We'll let the user hit Enter or rely on App update, OR we can force a refresh if desired. 
      // For best UX, let's assume App might need an update or we manually trigger refresh.
      // But looking at App.tsx, it might not auto-refresh on sortBy change. Let's rely on the user clicking refresh or adding sortBy to dependency array in App.
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              ...
              {/* Sort Controls */}
              <div className="hidden sm:flex bg-app-surface rounded-full p-1 border border-app-border">
                  <button 
                    onClick={() => handleSortChange('random')}
                    className={`p-1.5 rounded-full transition-all ${filters.sortBy === 'random' ? 'bg-app-accent text-white shadow-sm' : 'text-app-muted hover:text-app-text'}`}
                  ...
                  <button 
                    ...
                    className={`p-1.5 rounded-full transition-all ${filters.sortBy === 'top' ? 'bg-app-accent text-white shadow-sm' : 'text-app-muted hover:text-app-text'}`}
                    title="Top Rated (Likes)"
                  >
                      <TrendingUp className="w-4 h-4" />
                  </button>
                  <button 
                    onClick={() => handleSortChange('latest')}
                    className={`p-1.5 rounded-full transition-all ${filters.sortBy === 'latest' ? 'bg-app-accent text-white shadow-sm' : 'text-app-muted hover:text-app-text'}`}
                    title="Latest"
                  >
