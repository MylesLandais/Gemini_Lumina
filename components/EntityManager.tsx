...
import { 
  Users, Save, Plus, Trash2, Instagram, Twitter, 
  ExternalLink, ArrowLeft, Search, Network, Tag, Camera, Youtube, Globe, Link2,
  Eye, EyeOff, Archive, MessageSquare, MessagesSquare, Clover, Download, Upload, ShoppingBag, Database
} from 'lucide-react';
...
export const EntityManager: React.FC<EntityManagerProps> = ({ onBack }) => {
  ...
  const getPlatformIcon = (platform: PlatformType) => {
      switch (platform) {
          ...
          case 'kemono':
          case 'coomer': return <Archive className="w-3 h-3" />;
          case 'simpcity': return <MessageSquare className="w-3 h-3" />;
          case 'forum': return <MessagesSquare className="w-3 h-3" />;
          case 'pixiv': return <span className="font-bold text-xs text-blue-400">P</span>;
          case '4chan': return <Clover className="w-3 h-3" />;
          case 'imageboard': return <Clover className="w-3 h-3 opacity-75" />;
          case 'depop':
          case 'vinted':
          case 'grailed': return <ShoppingBag className="w-3 h-3" />;
          case 'myfigurecollection': return <Database className="w-3 h-3" />;
          default: return <Globe className="w-3 h-3" />;
      }
  };

  const getPlaceholderForPlatform = (platform: PlatformType) => {
      if (platform === 'reddit') return 'Subreddit Name';
      if (platform === '4chan') return 'Board (e.g. b, g)';
      if (platform === 'imageboard') return 'Domain/Board (e.g. 8kun.top/p)';
      if (platform === 'simpcity') return 'Thread ID / URL';
      if (platform === 'kemono' || platform === 'coomer') return 'Creator ID / URL';
      if (platform === 'forum') return 'Forum Profile URL';
      if (platform === 'pixiv') return 'Artist ID';
      if (['depop', 'vinted', 'grailed', 'ebay'].includes(platform)) return 'Search Term / Handle';
      if (platform === 'myfigurecollection') return 'Item ID / URL';
      return 'Handle / ID';
  };
      ...
      <div className="w-full md:w-80 border-r border-app-border flex flex-col h-full bg-app-bg">
        <div className="p-4 border-b border-app-border flex items-center justify-between">
          ...
          <div className="flex gap-1">
             <button onClick={handleImportClick} className="p-2 hover:bg-app-surface text-app-muted hover:text-app-text rounded-full transition-colors" title="Import Graph (JSON)">
                <Upload className="w-4 h-4" />
             </button>
             <button onClick={handleExportGraph} className="p-2 hover:bg-app-surface text-app-muted hover:text-app-text rounded-full transition-colors" title="Export Graph (JSON)">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(identities)
            .filter((p: IdentityProfile) => p.name.toLowerCase().includes(filterText.toLowerCase()))
            .map((profile: IdentityProfile) => (
            <div 
              key={profile.id}
                ...
                <div className="flex flex-col gap-2 bg-app-bg/50 p-3 rounded-lg border border-app-border/50">
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            ...
                            <option value="imageboard">Imageboard (Generic)</option>
                            <option value="myfigurecollection">MFC (MyFigureCollection)</option>
                            <option value="instagram">Instagram</option>
                            <option value="twitter">Twitter / X</option>
                            <option value="tiktok">TikTok</option>
                            <option value="youtube">YouTube</option>
                            <option value="pixiv">Pixiv</option>
                            <option value="forum">Forum (URL)</option>
                            <option value="kemono">Kemono.su</option>
                            <option value="coomer">Coomer.party</option>
                            <option value="simpcity">SimpCity</option>
                        ...
                        <input 
                            ...
                            placeholder={getPlaceholderForPlatform(newSourcePlatform)}
                   ...
                   <div className="flex flex-col gap-2 bg-app-bg/50 p-3 rounded-lg border border-app-border/50">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        >
                            ...
                            {Object.values(identities)
                                .filter((id: IdentityProfile) => id.id !== formState.id) // Exclude self
                                .map((id: IdentityProfile) => (
