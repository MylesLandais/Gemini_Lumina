...
import { AcquisitionBoard } from './components/AcquisitionBoard';
import { Library } from './components/Library';
import { Reader } from './components/Reader';
import { FilterState, INITIAL_FILTERS, FeedItem as FeedItemType, SavedBoard, Theme, AppView, LibraryItem } from './types';
...
export default function App() {
  ...
  // Followed Tags (Persistent)
  const [followedTags, setFollowedTags] = useState<string[]>(() => {
    try {
        const saved = localStorage.getItem('lumina_followed_tags');
  ...
  useEffect(() => {
    localStorage.setItem('lumina_followed_tags', JSON.stringify(followedTags));
  }, [followedTags]);
  ...
  const handleSaveBoard = () => {
    let defaultName = 'My Custom Feed';
    if (filters.persons.length > 0) defaultName = filters.persons.join(' & ');
    else if (filters.tags.length > 0) defaultName = filters.tags.join(', ');
  ...
  const refreshFeed = useCallback(async () => {
    ...
    const [items] = await Promise.all([
      generateFeedItems(filters.searchQuery, filters.persons, filters.sources, filters.tags, filters.sortBy),
      minTimePromise
    ]);
  ...
  }, [filters.persons, filters.sources, filters.tags]);
  ...
  const renderView = () => {
      switch (currentView) {
          case 'admin':
              return <EntityManager onBack={() => setCurrentView('feed')} />;
          case 'drafts':
              return <DraftEditor />;
          case 'sourcing':
              return <AcquisitionBoard />;
          case 'library':
              return <Library onOpenReader={handleOpenReader} />;
          case 'reader':
              return selectedLibraryItem ? <Reader item={selectedLibraryItem} onBack={() => setCurrentView('library')} /> : <Library onOpenReader={handleOpenReader} />;
          case 'feed':
          default:
              return (
                <>
                  ...
                  {loading && feedItems.length === 0 ? (
                    ...
                    <>
                      {feedItems.length === 0 ? (
                        ...
                        <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 space-y-4 mx-auto max-w-7xl pb-12">
                          {feedItems.map((item) => (
                            <FeedItem 
                                key={item.id} 
                                item={item} 
